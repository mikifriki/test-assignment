...
export class PageService {
	...
	handlePage(dataSource: MatTableDataSource<any>, event: any, totalPages: number, service, paginator: MatPaginator): Subscription {
		if (dataSource.data.length === totalPages) {
			return;
		}
	...
	removeSelected(selected, dataSource, service, paginator): SelectionModel<any> {
		return selected.forEach(book => {
			if (book.status !== 'AVAILABLE' && service === this.bookService) {
				return;
			}
			const elementPos = dataSource.data.map((x) => {
				return x.id;
			}).indexOf(book.id);
